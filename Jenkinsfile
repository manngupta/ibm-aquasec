    ...
    stages {
        stage('Scan Base Image') {
            steps {
                ...
                sh label: '', script: 'trivy client --remote http://3.86.7.235:8082 --format template --template @junit.tpl -o base-image.xml centos:8'
        ...
        stage('Scan App Image') {
            steps {
                sh label: '', script: 'trivy client --remote http://3.86.7.235:8082 --format template --template @junit.tpl -o app-image.xml thinknyx/devopsinaction:1.0'
